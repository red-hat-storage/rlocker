...
def retrieve_resource_entrypoint(request, search_string):
    ...
    try:
        ...
        print(
            f"Search String: {search_string} matches a Lockable resource by name. ID: {by_name.id} \n"
            f"Preparing Request ..."
        )
        by_name_url = reverse(
        ...
        if by_label:
            ...
            print(
                f"Search String: {search_string} is a valid label.  \n"
                f"Preparing Request ..."
            )
...
def rqueues_view(request):
    ...
    if request.method == "GET":
        ...
        if status_matches is not None:
            ...
            queryset = Rqueue.objects.filter(status=status_matches.upper()).order_by(
                "-id"
            )
