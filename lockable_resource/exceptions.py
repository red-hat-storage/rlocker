...
class FreeResourceNotAvailableException(Exception):
    def __init__(self, attempted_string):
        print(
            f"There are no free resources available that matches a label or a name: {attempted_string}"
        )
...
class LockWithoutSignoffException(Exception):
    def __init__(self, obj):
        print(
            f"Cannot lock resource {obj.name} without signoff. "
...
class InvalidLabelException(Exception):
    ...
    def __init__(self, label):
        print(
            f"Invalid Label {label}. This label does not exists across all the lockable resources"
